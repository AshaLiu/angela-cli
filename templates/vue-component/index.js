...
module.exports = function(creator, options, callback) {
  ...
  creator.copyTpl('index', path.join(srcPath, 'index.js'), {
    name
  });
  creator.copyTpl('vue', path.join(srcPath, name + '.vue'), {
    name
  });
  creator.copyTpl('webpack.config', path.join(projectPath, 'webpack.config.js'), {
    name
  });
